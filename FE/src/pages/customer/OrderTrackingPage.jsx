...
import { ImagePlus, Loader2, Package, Truck } from 'lucide-react'
import Badge from '../../components/common/Badge'
import { getOrders, uploadDeliveryImage } from '../../features/orders/order.api'
...
export default function OrderTrackingPage() {
  ...
  const [deliveryImageUploading, setDeliveryImageUploading] = useState(false)
  ...
  const canUploadDeliveryImage = selectedOrderStatus === 'COMPLETED'
  ...
  const handleDeliveryImageChange = async (event) => {
    const file = event.target.files?.[0]
    event.target.value = ''
    if (!selectedOrder || !file) return
    const allowedTypes = ['image/jpeg', 'image/png', 'image/webp']
    if (!allowedTypes.includes(file.type)) {
      setDeliveryImageError('Chỉ hỗ trợ ảnh JPG, PNG hoặc WEBP.')
      return
    }
    if (file.size > 3 * 1024 * 1024) {
      setDeliveryImageError('Ảnh nhận hàng không được vượt quá 3MB.')
      return
    }

    setDeliveryImageUploading(true)
    setDeliveryImageError('')
    try {
      const updatedOrder = await uploadDeliveryImage(selectedOrder.id, file)
      replaceOrder(updatedOrder)
    } catch (err) {
      setDeliveryImageError(err?.message || 'Không thể tải ảnh nhận hàng. Vui lòng thử lại.')
    } finally {
      setDeliveryImageUploading(false)
    }
  }
            ...
            <div className="bg-surface-container-lowest rounded-xl p-6 ambient-shadow sticky top-28">
              ...
              <div className="mt-6 border-t border-outline-variant/20 pt-5">
                <div className="flex items-center justify-between gap-3 mb-3">
                  <h3 className="font-label-sm uppercase tracking-wider text-on-surface-variant">Ảnh nhận hàng</h3>
                  {canUploadDeliveryImage && (
                    <label className={`inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium transition-colors ${
                      deliveryImageUploading
                        ? 'cursor-wait bg-surface-container text-on-surface-variant'
                        : 'cursor-pointer bg-primary text-on-primary hover:opacity-90'
                    }`}>
                      {deliveryImageUploading ? <Loader2 size={13} className="animate-spin" /> : <ImagePlus size={13} />}
                      {deliveryImageUploading ? 'Đang tải...' : 'Tải ảnh lên'}
                      <input
                        type="file"
                        accept="image/jpeg,image/png,image/webp"
                        className="sr-only"
                        disabled={deliveryImageUploading}
                ...
                {canUploadDeliveryImage ? (
                  <p className="mb-3 text-xs leading-relaxed text-on-surface-variant">
                    Sau khi nhận hàng, bạn có thể lưu ảnh kiện hàng để đối soát giao nhận và bảo hành.
                  </p>
                ) : (
                  <p className="mb-3 text-xs leading-relaxed text-on-surface-variant">
                    Ảnh nhận hàng sẽ được mở khi đơn chuyển sang Đã giao thành công.
                  </p>
                )}
                {deliveryImageError && (
                  <p role="alert" className="mb-3 rounded-lg border border-error/20 bg-error-container/20 px-3 py-2 text-xs text-error">
                    {deliveryImageError}
                  </p>
                )}
                {selectedOrder.deliveryImages?.length > 0 ? (
                  <div className="grid grid-cols-3 gap-2">
                    {selectedOrder.deliveryImages.map((image) => (
                      <a
                        key={image.id}
                  ...
                  <div className="rounded-lg border border-dashed border-outline-variant/30 bg-surface-container-low px-4 py-5 text-center text-xs text-on-surface-variant">
