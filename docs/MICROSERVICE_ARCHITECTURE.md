...
Quyền sở hữu dữ liệu được phân định rạch ròi theo bảng dưới đây (theo **DATA_MODEL_DOCUMENTATION**):

|| Service | Cơ sở dữ liệu | Quyền sở hữu bảng (Tables Owned) | Trách nhiệm chính |
|| :--- | :--- | :--- | :--- |
|| `auth-service` | `auth_db` (Postgres) | `users` | Lưu thông tin định danh và tài khoản hệ thống (email, password_hash, full_name, provider, provider_id, role). |
|| `user-service` | `user_db` (Postgres) | `customer_style_profiles`, `delivery_addresses` | Lưu hồ sơ sinh trắc học, gu thẩm mỹ (style_personas) và sổ địa chỉ giao hàng của khách hàng. |
|| `product-service` | `product_db` (Postgres) | `categories`, `products`, `product_variants`, `product_images` | Quản lý danh mục cây, sản phẩm gốc, biến thể (SKU, size, color, material, price_override) và hình ảnh sản phẩm. |
|| `cart-service` | `cart_db` (Postgres) | `shopping_carts`, `cart_items` | Lưu giỏ hàng hiện hành, hỗ trợ gắn cờ `is_ai_recommended` và `source_bundle_id` cho conversion tracking. |
|| `order-service` | `order_db` (Postgres) | `orders`, `order_items` | Quản lý vòng đời đơn hàng (Saga), snapshot giá tại mua (`price_at_purchase`), và tracking AI conversion. |
|| `payment-service` | `payment_db` (Postgres) | `transactions` | Lưu trữ lịch sử giao dịch thanh toán (online/COD), hỗ trợ refund. |
|| `ai-agent-service` | `ai_db` (Postgres) <br> **Qdrant** (Vector) <br> **Neo4j** (Graph) | `chat_sessions`, `chat_messages`, `ai_curated_bundles`, `ai_curated_bundle_items`, `ai_analytics_logs`, `ai_index_jobs` | **Không sở hữu dữ liệu gốc sản phẩm/khách hàng.** Chỉ sở hữu log hội thoại, bundles do AI tạo, analytics interaction logs và quản lý index job. Qdrant lưu vector embeddings; Neo4j lưu Knowledge Graph (Product, Category, Color, Material, Style, Occasion, Season, BodyType, Outfit). Khi cần giá/đơn hàng thực tế → gọi API runtime sang service sở hữu. |
|| `notification-service` | `notification_db` (Postgres) | `notification_logs` | Lưu trữ lịch sử gửi email, SMS, push notification. |
...
#### A. `auth_db` (`auth-service`)
```sql
CREATE TABLE users (
    id VARCHAR(50) PRIMARY KEY,                 -- UUID
    email VARCHAR(100) UNIQUE NOT NULL,
    password_hash VARCHAR(255),                 -- Nullable cho SSO
    full_name VARCHAR(150),
    provider VARCHAR(20) NOT NULL DEFAULT 'LOCAL', -- LOCAL, GOOGLE, FACEBOOK
    provider_id VARCHAR(100),                   -- ID từ SSO provider
    role VARCHAR(20) NOT NULL DEFAULT 'CUSTOMER', -- CUSTOMER, ADMIN
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
```

#### B. `user_db` (`user-service`)
```sql
-- Hồ sơ phong cách & sinh trắc học khách hàng (1:1 với users)
CREATE TABLE customer_style_profiles (
    ...
    body_morphology VARCHAR(50),               -- Rectangle, Hourglass, Pear, Inverted Triangle, Apple
    preferred_fit VARCHAR(30),                 -- Slim-fit, Regular, Oversized
    style_personas JSONB,                      -- Mảng JSON: ["Minimalist", "Streetwear"]
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE delivery_addresses (
    ...
    address_line TEXT NOT NULL,                -- Số nhà, ngõ, đường
    city VARCHAR(100) NOT NULL,                -- Dùng để lấy thời tiết realtime
    is_default BOOLEAN DEFAULT FALSE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
```

#### C. `product_db` (`product-service`)
```sql
-- Danh mục sản phẩm (cấu trúc cây đệ quy)
...
CREATE TABLE products (
    ...
    description TEXT,                          -- Dùng cho AI embedding
    base_price DECIMAL(12, 2) NOT NULL,
    aesthetic_style VARCHAR(50),               -- Casual, Streetwear, Vintage, Formal...
    target_demographic VARCHAR(20),            -- Men, Women, Unisex, Kids
    seasonal_property VARCHAR(20),             -- Spring, Summer, Autumn, Winter, All-Season
...
CREATE TABLE product_variants (
    ...
    material VARCHAR(50),                      -- Cotton, Linen, Silk, Jeans, Wool...
    price_override DECIMAL(12, 2)              -- Nullable: giá riêng cho variant
);
...
CREATE TABLE product_images (
    id SERIAL PRIMARY KEY,
    product_id VARCHAR(50) NOT NULL REFERENCES products(id) ON DELETE CASCADE,
    image_url VARCHAR(500) NOT NULL,           -- URL CDN/MinIO
    is_primary BOOLEAN DEFAULT FALSE
);
```

#### D. `cart_db` (`cart-service`)
```sql
-- Giỏ hàng (1 user = 1 cart)
CREATE TABLE shopping_carts (
    id VARCHAR(50) PRIMARY KEY,                -- user_id (cho user đã đăng nhập) hoặc guest_session_id
    user_id VARCHAR(50) UNIQUE REFERENCES users(id), -- NULL cho guest
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE cart_items (
    ...
    is_ai_recommended BOOLEAN DEFAULT FALSE,   -- Thêm từ AI bundle
    source_bundle_id VARCHAR(50) REFERENCES ai_curated_bundles(id), -- FK đến AI bundle
    added_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
```

#### E. `order_db` (`order-service`)
```sql
-- Đơn hàng
CREATE TABLE orders (
    ...
    order_status VARCHAR(30) NOT NULL DEFAULT 'PENDING', -- PENDING, PROCESSING, COMPENSATING_ROLLBACK, FULFILLED, CANCELLED
    shipping_address TEXT NOT NULL,            -- Snapshot địa chỉ lúc đặt
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Chi tiết đơn hàng (snapshot giá)
CREATE TABLE order_items (
    id VARCHAR(50) PRIMARY KEY,
    order_id VARCHAR(50) NOT NULL REFERENCES orders(id) ON DELETE CASCADE,
    variant_id VARCHAR(50) NOT NULL REFERENCES product_variants(id),
    quantity INT NOT NULL CHECK (quantity > 0),
    price_at_purchase DECIMAL(12, 2) NOT NULL, -- Snapshot giá tại mua
    is_ai_conversion BOOLEAN DEFAULT FALSE,    -- Conversion từ AI recommendation
    source_bundle_id VARCHAR(50) REFERENCES ai_curated_bundles(id), -- FK đến AI bundle gốc
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
```

#### F. `payment_db` (`payment-service`)
```sql
CREATE TABLE transactions (
    ...
    method VARCHAR(30) NOT NULL,               -- online_simulated, cod
    status VARCHAR(30) NOT NULL,               -- PENDING, COMPLETED, FAILED, REFUNDED
    transaction_ref VARCHAR(100),              -- ID từ cổng thanh toán
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
```

#### H. `ai_db` (`ai-agent-service`)
```sql
-- Phiên hội thoại với AI
CREATE TABLE chat_sessions (
    id UUID PRIMARY KEY,
    user_id VARCHAR(50) REFERENCES users(id),  -- NULL cho guest
    context_weather_temp DECIMAL(4, 1),        -- Nhiệt độ realtime
    context_weather_condition VARCHAR(30),     -- Rainy, Sunny, Cloudy...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE chat_messages (
    ...
    has_product_block BOOLEAN DEFAULT FALSE,   -- Có product block không
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE ai_curated_bundles (
    ...
    justification_summary TEXT,                -- Lý do phối đồ
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Chi tiết sản phẩm trong bundle
...
-- Analytics interaction logs (Impression, Click, Add_to_Cart)
CREATE TABLE ai_analytics_logs (
    id VARCHAR(50) PRIMARY KEY,
    user_id VARCHAR(50) NOT NULL REFERENCES users(id),
    bundle_id VARCHAR(50) NOT NULL REFERENCES ai_curated_bundles(id),
    interaction_type VARCHAR(30) NOT NULL,     -- IMPRESSION, CLICK, ADD_TO_CART
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Quản lý job đồng bộ index AI
CREATE TABLE ai_index_jobs (
    id VARCHAR(50) PRIMARY KEY,
    target_type VARCHAR(30) NOT NULL,          -- PRODUCT, INVENTORY, RULE
...
Vòng đời của các đối tượng nghiệp vụ được quản lý chặt chẽ qua các trạng thái (tuân thủ **DATA_MODEL_DOCUMENTATION**):
...
B. Đơn hàng (Order State Machine) - Theo DATA_MODEL_DOCUMENTATION
+-----------+    Thanh toán lỗi/Hủy    +-----------+
|  PENDING  |------------------------->| CANCELLED |
+-----+-----+                          +-----------+
      |
...
| PROCESSING|---------------------->| COMPENSATING_ |--------------------->|  FULFILLED |---------------------->| [Hoàn tất] |
|          |                         |  ROLLBACK   |                      |            |                       |            |
+-----------+                       +-------------+                      +-----------+                       +-----------+

Lưu ý: Theo DATA_MODEL_DOCUMENTATION, các trạng thái đơn hàng là:
PENDING → PROCESSING → COMPENSATING_ROLLBACK (nếu lỗi) / FULFILLED (thành công) → CANCELLED (bất kỳ lúc nào khi hủy)

C. Thanh toán (Payment State Machine)
+-----------+    Lỗi kết nối/Từ chối   +-----------+
|  PENDING  |------------------------->|  FAILED   |
+-----+-----+                          +-----------+
      |
...
|| `target_type` | VARCHAR(30) | NOT NULL | Loại thực thể cần index (`PRODUCT`, `RULE`). ||
...
[Frontend] --(POST /orders {items, total, ...})--> [API Gateway] --(Chuyển tiếp)--> [order-service]
                                                                                   ...
                                                                                   [order-service]
                                                                                         |
                                                                                         |-- 4. Cập nhật đơn hàng sang 'FULFILLED' (hoặc 'CANCELLED' nếu lỗi)
                                                                                         v
[Frontend] <--(Trả về đơn hàng đã xử lý thành công)--------------------------------------+
```
...
[Frontend] --(POST /payment/checkout)--> [API Gateway] --(Chuyển tiếp)--> [payment-service]
                                                                        ...
                                                                        [order-service]
                                                                               ...
                                                                               |   - Cập nhật order sang 'FULFILLED'.
                                                                               v
[Frontend] <--(Trả về kết quả thanh toán thành công, đơn FULFILLED)-----------+
```
