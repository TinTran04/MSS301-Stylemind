...
- `/profile` (alias `/style-profile`) là trang tài khoản khách hàng: hiển thị thông tin đăng nhập, tab riêng cho style profile và địa chỉ giao hàng; form địa chỉ chỉ mở khi người dùng chủ động thêm/chỉnh sửa, CRUD địa chỉ giao hàng qua `GET/POST/PUT/DELETE /api/v1/users/addresses`. Form chọn `provinceCode`/`wardCode` từ lookup `/api/v1/users/administrative/**`, nhập phone recipient và shipping note; backend là authority và trả `validationStatus`.
- `/checkout` tải các địa chỉ đã lưu, chỉ cho chọn `validationStatus=VALID`, và gửi `POST /api/v1/orders` với `addressId` + `paymentMethod`. Không gửi free-text `shippingAddress`, không gọi User Service internal hay service port trực tiếp.
